...
var ArrayKnowledgeCephResourcesTitleText = [
	...
	"2.1、安装时间同步服务[*]\n	#参考chrony部署\n2.2、配置设备免密互通服务ssh[master安装]\n    [root@cephmaster ~]# su - cephuser\n    # 第一步：本地节点生成密钥对\n    [root@master ~]# ssh-keygen\n    # 第二步：授权远程节点\n    [root@master ~]# ssh-copy-id 192.168.2.7\n    [root@master ~]# ssh-copy-id 192.168.2.8\n    #[root@master ~]# ssh-copy-id 192.168.2.*",
...
var ArrayKnowledgeLinuxInstructResourcesTitleText = [
	...
	"cd:进入指定目录",
	"mv:重命名文件|移动文件\n-------------------------------------------------------------------------------------\n相关命令:\n-------------------------------------------------------------------------------------\n重命名该文件:\n	[root@* ~]mv a b\n移动/tmp/A文件到/usr/:\n	[root@* ~]mv /tmp/A /usr/",
	"cp:拷贝文件\n-------------------------------------------------------------------------------------\n相关命令:\n-------------------------------------------------------------------------------------\n将当前目录下jdk-8u40-linux-x64.gz文件拷贝至/tmp/\n	[root@* ~]cp -R jdk-8u40-linux-x64.gz /tmp/",
	"cat:查看文本文件\n-------------------------------------------------------------------------------------\n相关命令:\n-------------------------------------------------------------------------------------\n查看文件多少行:\n	[root@* ~]cat 文件名 | wc -l",
